"""Unit test for ReadTool (read)."""
import pytest
from pathlib import Path
from axon.tools import ReadTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_read_tool_initialization(workspace: Path, settings):
    tool = ReadTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_read_tool_execution(workspace: Path, settings):
    ...
    tool = ReadTool()
    try:
        res = tool.run({"path": "sample.txt"}, ctx)
