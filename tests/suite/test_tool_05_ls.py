"""Unit test for LsTool (ls)."""
import pytest
from pathlib import Path
from axon.tools import LsTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_ls_tool_initialization(workspace: Path, settings):
    tool = LsTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_ls_tool_execution(workspace: Path, settings):
