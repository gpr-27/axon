"""Unit test for TaskTool (task)."""
import pytest
from pathlib import Path
from axon.tools import TaskTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_task_tool_initialization(workspace: Path, settings):
    tool = TaskTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_task_tool_execution(workspace: Path, settings):
    ...
    tool = TaskTool()
    try:
        res = tool.run({"prompt": "analyze data", "description": "task"}, ctx)
