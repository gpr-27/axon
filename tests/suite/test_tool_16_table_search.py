"""Unit test for TableSearchTool (table_search)."""
import pytest
from pathlib import Path
from axon.tools import TableSearchTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_table_search_tool_initialization(workspace: Path, settings):
    tool = TableSearchTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_table_search_tool_execution(workspace: Path, settings):
    ...
    tool = TableSearchTool()
    try:
        res = tool.run({"query": "data", "path": "sample.txt"}, ctx)
