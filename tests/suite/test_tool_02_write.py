"""Unit test for WriteTool (write)."""
import pytest
from pathlib import Path
from axon.tools import WriteTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_write_tool_initialization(workspace: Path, settings):
    tool = WriteTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_write_tool_execution(workspace: Path, settings):
    ...
    tool = WriteTool()
    try:
        res = tool.run({"path": "sample.txt", "content": "hello"}, ctx)
