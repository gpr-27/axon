"""Unit test for DeepResearchTool (deep_research)."""
import pytest
from pathlib import Path
from axon.tools import DeepResearchTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_deep_research_tool_initialization(workspace: Path, settings):
    tool = DeepResearchTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_deep_research_tool_execution(workspace: Path, settings):
    ...
    tool = DeepResearchTool()
    try:
        res = tool.run({"query": "Algorithms", "depth": "quick"}, ctx)
