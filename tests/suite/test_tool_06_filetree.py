"""Unit test for FileTreeTool (filetree)."""
import pytest
from pathlib import Path
from axon.tools import FileTreeTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_filetree_tool_initialization(workspace: Path, settings):
    tool = FileTreeTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_filetree_tool_execution(workspace: Path, settings):
    ...
    tool = FileTreeTool()
    try:
        res = tool.run({"path": ".", "max_depth": 2}, ctx)
