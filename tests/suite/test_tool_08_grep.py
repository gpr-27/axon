"""Unit test for GrepTool (grep)."""
import pytest
from pathlib import Path
from axon.tools import GrepTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_grep_tool_initialization(workspace: Path, settings):
    tool = GrepTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_grep_tool_execution(workspace: Path, settings):
