"""Unit test for HttpTool (http)."""
import pytest
from pathlib import Path
from axon.tools import HttpTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_http_tool_initialization(workspace: Path, settings):
    tool = HttpTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_http_tool_execution(workspace: Path, settings):
    ...
    tool = HttpTool()
    try:
        res = tool.run({"url": "https://example.com", "method": "GET"}, ctx)
