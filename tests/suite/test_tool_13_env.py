"""Unit test for EnvTool (env)."""
import pytest
from pathlib import Path
from axon.tools import EnvTool
from axon.tools.base import ToolContext
from axon.agent.state import FileState, TodoState

def test_env_tool_initialization(workspace: Path, settings):
    tool = EnvTool()
    assert tool.name is not None
    assert tool.description is not None
    assert tool.schema is not None

def test_env_tool_execution(workspace: Path, settings):
